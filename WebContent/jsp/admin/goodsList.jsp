...
<html>
	...
	$(function() {
		...
		function showDialogForEdit(id){
			window.showModalDialog('${pageContext.request.contextPath }/goodsAction_updateGoodsDialog.action?id='+id,'','dialogWidth=450px;dialogHeight=350px;');
	}
	...
	<body>
		<!-- 综合查询操作层 -->
		<div id="selectDiv">
			<s:form action="goodsAction_goodsList.action" theme="simple" target="mainFrame" method="post">
				<fieldset style="width:80%">
					<legend>查询条件</legend>
				商品名:<s:textfield name="name" cssClass="textStyle"/>
		...
		<table class="ta">	
			...
			<c:forEach items="${goodslist }" var="goods" varStatus="status">
				<tr>
					...
					<td><a style="border-width:0px" href="${path }/goodsAction_goodsDelete.action?goods.id=${goods.id }"><img src="${path }/images/delete.png" alt="删除"/></a></td>
					<td>产品_${goods.id }</td>
					<td>${goods.name }</td>
