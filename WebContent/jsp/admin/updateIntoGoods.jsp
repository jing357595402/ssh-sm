...
<html>
	...
	<body>
		...
		<form action="${path }/intoGoodsAction_updateIntoGoods.action" target="mainFrame" onsubmit="javascript:window.close();">
		<table class="ta" width="200px" >	
		<tr >
			<td>进货时间</td>
			<td><input type="text" name="intoDate" id="intoDate" value="${request.intoGoods.intoDate }"/>
				<input type="hidden" name="id" id="intoDate" value="${request.intoGoods.id }"/>
			</td>
		</tr>
		<tr >
			<td>进货数量</td>
			<td><input type="text" name="num"  value="${request.intoGoods.num }"/>
			</td>
		</tr>
		
		<tr >
			<td>
				<s:select list="%{#request.map}" name="goods.id" headerKey="-1" headerValue="请选择"  label="进货商品"></s:select>
            </td>
		</tr>
		<tr>
