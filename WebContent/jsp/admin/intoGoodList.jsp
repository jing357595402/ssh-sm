...
<html>
	...
	$(function() {
	 ...
	 $("#beginDate").datepicker({ dateFormat: 'yy-m-d' });
		...
		function showDialogForEdit(id){
			window.showModalDialog('${path }/intoGoodsAction_updateIntoGoodsDialog.action?intoGoods.id='+id,'','dialogWidth=450px;dialogHeight=350px;');
	}
	...
	<body>
		<!-- 综合查询操作层 -->
		<div id="selectDiv">
			<s:form action="intoGoodsAction_intoGoodsList.action" theme="simple" target="mainFrame" >
				<fieldset style="width:80%">
					<legend>查询条件</legend>
				商品名:<s:select list="%{#request.map}" name="goods.id" headerKey="-1" headerValue="请选择"  label="商品名"></s:select>
