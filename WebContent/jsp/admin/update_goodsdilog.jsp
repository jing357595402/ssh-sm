...
<html>
	...
	<body>
		...
		<form action="${path }/goodsAction_updateGoods.action" target="mainFrame" onsubmit="javascript:window.close();">
		<table class="ta" width="200px">	
		<tr >
			<td>商品名</td>
			<td><input type="text" name="name" value="${request.goods.name }"/></td>
		</tr>
		<tr >
			<td>进货价格</td>
			<td><input type="text" name="inputPrise" value="${request.goods.inputPrise }"/>
			<input type="hidden" name="id" value="${request.goods.id }"/>
			</td>
		</tr>
		<tr >
			<td>卖出价格</td>
			<td><input type="text" name="outputPrise" value="${request.goods.outputPrise }"/></td>
		</tr>
		<tr >
			<td>剩余数量</td>
			<td><input type="text" name="remainNum" value="${request.goods.remainNum }"/></td>
		</tr>
		<tr >
			<td>来自厂商</td>
			<td>
