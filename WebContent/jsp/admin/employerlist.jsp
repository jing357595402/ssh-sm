...
<html>
	...
	$(function() {
		...
		function showDialogForEdit(id){
			window.showModalDialog('${pageContext.request.contextPath }/employerAction_updateEmployerDialog.action?employer.id='+id,'','dialogWidth=450px;dialogHeight=350px;');
	}
	...
	<body>
		<!-- 综合查询操作层 -->
		<div id="selectDiv">
			<s:form action="employerAction_emplyerList.action" theme="simple" target="mainFrame" >
				<fieldset style="width:80%">
					<legend>查询条件</legend>
				姓名:<s:textfield name="username" cssClass="textStyle"/>
				家庭住址:<s:textfield name="adress" cssClass="textStyle"/>
				电话:<s:textfield name="telephone" cssClass="textStyle"/>
		...
		<table class="ta">	
			...
			<c:forEach items="${list }" var="emp" varStatus="status">
				<tr bgColor="${status.index%2==0?'#e5fee2':'#d6fdd0' }">
					<td><img src="${pageContext.request.contextPath }/images/edit2.png" alt="编辑" onclick="showDialogForEdit(${emp.id})"/></td>
					<td><a style="border-width:0px" href="${pageContext.request.contextPath }/employerAction_deleteEmployer.action?employer.id=${emp.id }"><img src="${pageContext.request.contextPath }/images/delete.png" alt="删除"/></a></td>
					<td>${emp.id }</td>
					<td>${emp.username }</td>
					<td>${emp.adress }</td>
					<td>${emp.telephone}</td>
